...
Set up the upstream remote (once per clone):
...
Create a sync branch and rebase our `main` on upstream:
...
Verify we have not reintroduced OpenSSL or Sentry:
...
docker run --rm \
  ...
  bash -lc '
    ...
    dnf -y install curl git pkgconfig openssl-devel libcap-devel libseccomp-devel && \
...
- If you do not already have Docker installed, follow the official installation guides for your platform at the [Docker documentation](https://docs.docker.com/get-docker/).
